...
method init(self: Accumulator) =
  ...
  globalParams.add([
    Parameter(name: "add", kind: Float, min: 0.0, max: 1.0, default: 0.0, onchange: proc(newValue: float32, voice: int) =
      self.value += newValue
    ...
    Parameter(name: "max", kind: Float, min: 1.0, max: 1000.0, default: 4.0, onchange: proc(newValue: float32, voice: int) =
      self.max = newValue
      if self.value >= self.max:
        self.value = self.value mod self.max
      self.globalParams[2].value = self.value
    ),
    Parameter(name: "value", kind: Float, min: 0.0, max: 1000.0, default: 0.0, onchange: proc(newValue: float32, voice: int) =
      self.value = newValue
    ),
  ])
  setDefaults()
