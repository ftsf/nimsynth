...
method init(self: EADSRMachine) =
  ...
  globalParams.add([
    ...
    Parameter(kind: Float, name: "min", min: -10000'f, max: 10000.0'f, default: 0.0'f, onchange: proc(newValue: float32, voice: int) =
      self.min = newValue
    ),
    Parameter(kind: Float, name: "max", min: -10000'f, max: 10000.0'f, default: 1.0'f, onchange: proc(newValue: float32, voice: int) =
      self.max = newValue
    ),
