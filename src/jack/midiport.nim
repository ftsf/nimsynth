...
type
  JackMidiEvent* = object
    time*: jack_nframes      ## #*< Sample index at which event is valid 
    size*: csize               ## #*< Number of bytes of data in \a buffer 
    buffer*: ptr JackMidiData ## #*< Raw MIDI data 
...
proc jack_midi_get_event_count*(port_buffer: pointer): int {.importc.}
...
proc jack_midi_event_get*(event: ptr JackMidiEvent; port_buffer: pointer;
                         event_index: uint32): cint {.importc.}
...
proc jack_midi_clear_buffer*(port_buffer: pointer) {.importc.}
...
proc jack_midi_reset_buffer*(port_buffer: pointer) {.importc.}
...
proc jack_midi_max_event_size*(port_buffer: pointer): csize {.importc.}
...
proc jack_midi_event_reserve*(port_buffer: pointer; time: jack_nframes;
                             data_size: csize): ptr JackMidiData {.importc.}
...
proc jack_midi_event_write*(port_buffer: pointer; time: jack_nframes;
                           data: ptr JackMidiData; data_size: csize): cint {.importc.}
...
proc jack_midi_get_lost_event_count*(port_buffer: pointer): uint32 {.importc.}
## #@}
